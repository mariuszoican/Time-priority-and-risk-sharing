...
path="./MarginalQuotePanels/"
pathzf="./MarginalQuotePanels_ZeroFilled/"
...
if __name__ == "__main__":
    ...
    f=list_files[int(sys.argv[1])]

    print(f)

    temp = pd.read_csv(path + f, index_col=0)
    temp = temp.merge(mm_labels[['ap_account', '%slabel' % toselect]])
    ...
    start=dt.datetime(temp['datetime'][0].date().year,temp['datetime'][0].date().month,temp['datetime'][0].date().day,2,0,0)
    end=dt.datetime(temp['datetime'][0].date().year,temp['datetime'][0].date().month,temp['datetime'][0].date().day,18,0,0)
    ...
    for ss in list_ss:
        ...
        # get active traders
        set_ask = set(temp_ss[temp_ss.side == 'ask']['ap_account'])
        set_bid = set(temp_ss[temp_ss.side == 'bid']['ap_account'])

        not_bid_ID = set_ask - set_bid
        not_ask_ID = set_bid - set_ask

        tb = temp_ss[(temp_ss.side == 'ask') & (temp_ss['ap_account'].isin(not_bid_ID))]
        ta = temp_ss[(temp_ss.side == 'bid') & (temp_ss['ap_account'].isin(not_ask_ID))]
