...
sns.barplot(x="priority", y="quantity_resid_1", hue="side", palette='Paired',
            data=quotedata[quotedata.priority<=keep_top])

plt.xlabel('Queue position at best price', fontsize=20)
plt.ylabel('Residual marginal quote quantity',fontsize=20)

plt.title('Controls: inventory, depth on side, queue length, order imbalance, \n own queue ahead, spread, symbol and date FE', fontsize=20)
...
sns.barplot(x="priority", y="quantity_resid_2", hue="side", palette='Paired',
            data=quotedata[quotedata.priority<=keep_top])

plt.xlabel('Queue position at best price', fontsize=20)
plt.ylabel('Residual marginal quote quantity',fontsize=20)

plt.title('Controls: symbol and date FE', fontsize=20)
plt.xticks(rotation=45)
...
#
#
# sns.barplot(x="priority", y="quantity_resid_3", hue="side", palette='Paired',
#             data=quotedata[quotedata.priority<=keep_top])
#
# plt.xlabel('Queue position at best price', fontsize=20)
# plt.ylabel('Residual marginal quote quantity',fontsize=20)
#
# plt.title('Controls: depth on side, symbol and date FE', fontsize=20)
# plt.xticks(rotation=45)
#
# plt.legend(loc='best',frameon=False,fontsize=20)
...
sns.barplot(x="priority", y="quantity_resid_4", hue="side", palette='Paired',
            data=quotedata[quotedata.priority<=keep_top])

plt.xlabel('Queue position at best price', fontsize=20)
plt.ylabel('Residual marginal quote quantity',fontsize=20)

plt.title('Controls: inventory, depth on side, queue length, \n symbol and date FE', fontsize=20)
plt.xticks(rotation=45)
...
# Histogram of priority levels
snapshots=quotedata.groupby(['externalSymbol','datetime','side']).mean()['priority_levels'].reset_index()
...
sns.histplot(snapshots,x='priority_levels',cumulative=True,stat='probability',element='step',fill=False)
plt.xlabel('Queue length at best bid/best ask',fontsize=18)
plt.ylabel('Cumulative probability',fontsize=18)

#plt.show()
plt.savefig('../Output/CDF_queuelength.png',bbox_inches='tight')
...
size_diff=quotedata.pivot(index=['externalSymbol','datetime',
                                 'ap_account','quantile_inventory'],columns='side',
...
size_diff_nc=quotedata.pivot(index=['externalSymbol','datetime',
                                 'ap_account','quantile_inventory_nc'],columns='side',
